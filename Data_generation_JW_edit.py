...
colname = ['out','in','high','low','coil','comp','cond']
...
p = ax.scatter(final_df['out'], final_df['in'], final_df['coil'], c=final_df['state'])
ax.set_xlabel('Out temp(C)')
ax.set_ylabel('In temp(C)')
ax.set_zlabel('State temp(C)')
fig.colorbar(p, pad=0.15)
plt.show()

#%%

final_df.to_csv('./MT_label3.csv', index=False)
